...
class ApiService {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          ...
          throw ApiException(
              ...
              responseData[
                  'errorMessage']); // An exception is thrown on failure
        }
      } else {
        throw ApiException(response.statusCode ?? -1,
            'The network request failed'); //If the status code is not 200, an exception is thrown
      }
    } catch (error) {
  ...
  void _addCommonParams(Options options) {
    ...
    final deviceId =
        'your_device id'; // Let's say this is the unique identifier of the device
    final clientVersion =
        '1.0.0'; // Let's say this is the version number of the client
